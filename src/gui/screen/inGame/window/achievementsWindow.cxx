...
AchievementsWindow::AchievementsWindow(NodePath parent,playerAchievements player_achiev, float width, float height, string title, float pos_x, float pos_y) : Window(parent, width, height, title, pos_x, pos_y) {
	lb_andamento = new TextNode("lb_andamento");
	lb_andamento->set_text(ConfigVariableString("msg-andamento","ANDAMENTO"));
	//lb_andamento->set_font(FontPool::load_font("models/gui/fonts/suplexcomic-large"));
	np_lb_andamento = np_frame.attach_new_node(lb_andamento);
	...
	int lvl_senhor_mordida = player_achiev.get_smestrela();
	switch (lvl_senhor_mordida) {
		case 0:
			create_senhor_mordida_stars(white, white, white);
			hint_senhor_mordida_atual = new Hint(np_frame, lb_senhor_mordida, lb_senhor_mordida->get_np(), "hint_senhor_mordida", ConfigVariableString("msg-coma10", "Coma 10 vezes sem errar a mordida"));
	...
	lb_senhor_mordida_andamento = new TextNode("lb_senhor_mordida_andamento");
	simdunas_cat.debug()<<"test = "<<player_achiev.get_smcont() + " / 10"<<endl;
	stringstream out;
	if (lvl_senhor_mordida == 0) {
		out<<player_achiev.get_smcont();
		lb_senhor_mordida_andamento->set_text(out.str() + " / 10");
	}
	else if (lvl_senhor_mordida == 1) {
		out<<player_achiev.get_smcont();
	...
	int lvl_bom_de_boca = player_achiev.get_bbestrela();
	switch (lvl_bom_de_boca) {
		case 0:
			create_bom_de_boca_stars(white, white, white);
			hint_bom_de_boca_atual = new Hint(np_frame, lb_bom_de_boca, lb_bom_de_boca->get_np(), "hint_bom_de_boca", ConfigVariableString("msg-coma4dife", "Coma 4 espécies diferentes de insetos ou plantas."));
	...
	lb_bom_de_boca_andamento = new TextNode("lb_bom_de_boca_andamento");
	if (lvl_bom_de_boca == 0) {
		bom_de_boca_andamento<<player_achiev.get_bbcont();
		lb_bom_de_boca_andamento->set_text(bom_de_boca_andamento.str() + " / 4");
	}
	else if (lvl_bom_de_boca == 1) {
		bom_de_boca_andamento<<player_achiev.get_bbcont();
	...
	int lvl_sobrevivente = player_achiev.get_svestrela();
	switch (lvl_sobrevivente) {
		case 0:
			create_sobrevivente_stars(white, white, white);
			hint_sobrevivente_atual = new Hint(np_frame, lb_sobrevivente, lb_sobrevivente->get_np(), "hint_sobrevivente", ConfigVariableString("msg-sobreviva4", "Sobreviva por 4 meses."));
	...
	lb_sobrevivente_andamento = new TextNode("lb_sobrevivente_andamento");
	if (lvl_sobrevivente == 0) {
		sobrevivente_andamento<<player_achiev.get_svcont();
		lb_sobrevivente_andamento->set_text(sobrevivente_andamento.str() + " / 4");
	}
	else if (lvl_sobrevivente == 1) {
		sobrevivente_andamento<<player_achiev.get_svcont();
	...
	int lvl_reprodutor = player_achiev.get_reestrela();
	switch (lvl_reprodutor) {
		case 0:
			create_reprodutor_stars(white, white, white);
			hint_reprodutor_atual = new Hint(np_frame, lb_reprodutor, lb_reprodutor->get_np(), "hint_reprodutor", ConfigVariableString("msg-reproduza3", "Reproduza 3 vezes."));
	...
	lb_reprodutor_andamento = new TextNode("lb_reprodutor_andamento");
	if (lvl_reprodutor == 0) {
		reprodutor_andamento<<player_achiev.get_recont();
		lb_reprodutor_andamento->set_text(reprodutor_andamento.str() + " / 3");
	}
	else if (lvl_reprodutor == 1) {
		reprodutor_andamento<<player_achiev.get_recont();
	...
	int lvl_temperatura = player_achiev.get_tmestrela();
	switch (lvl_temperatura) {
		case 0:
			create_temperatura_stars(white, white, white);
			hint_temperatura_atual = new Hint(np_frame, lb_temperatura, lb_temperatura->get_np(), "hint_temperatura", ConfigVariableString("msg-temperatura45", "Mantenha sua temperatura entre 35 e 40 graus durante 45 segundos."));
	...
	lb_temperatura_andamento = new TextNode("lb_temperatura_andamento");
	if (lvl_temperatura == 0) {
		temperatura_andamento<<player_achiev.get_tmcont();
		lb_temperatura_andamento->set_text(temperatura_andamento.str() + " / 45");
	}
	else if (lvl_temperatura == 1) {
		temperatura_andamento<<player_achiev.get_tmcont();
	...
	int lvl_hidratacao = player_achiev.get_hiestrela();
	switch (lvl_hidratacao) {
		case 0:
			create_hidratacao_stars(white, white, white);
			hint_hidratacao_atual = new Hint(np_frame, lb_hidratacao, lb_hidratacao->get_np(), "hint_hidratacao", ConfigVariableString("msg-hidratacao70", "Mantenha sua hidratação maior que 70 durante 60 segundos."));
	...
	lb_hidratacao_andamento = new TextNode("lb_hidratacao_andamento");
	if (lvl_hidratacao == 0) {
		hidratacao_andamento<<player_achiev.get_hicont();
		lb_hidratacao_andamento->set_text(hidratacao_andamento.str() + " / 60");
	}
	else if (lvl_hidratacao == 1) {
		hidratacao_andamento<<player_achiev.get_hicont();
	...
	int lvl_energia = player_achiev.get_enestrela();
	switch (lvl_energia) {
		case 0:
			create_energia_stars(white, white, white);
			hint_energia_atual = new Hint(np_frame, lb_energia, lb_energia->get_np(), "hint_energia", ConfigVariableString("msg-energia70", "Mantenha sua energia maior que 70 durante 60 segundos."));
	...
	lb_energia_andamento = new TextNode("lb_energia_andamento");
	if (lvl_energia == 0) {
		energia_andamento<<player_achiev.get_encont();
		lb_energia_andamento->set_text(energia_andamento.str() + " / 60");
	}
	else if (lvl_energia == 1) {
		energia_andamento<<player_achiev.get_encont();
	...
	int lvl_intocavel = player_achiev.get_inestrela();
	switch (lvl_intocavel) {
		case 0:
			create_intocavel_stars(white, white, white);
			hint_intocavel_atual = new Hint(np_frame, lb_intocavel, lb_intocavel->get_np(), "hint_intocavel", ConfigVariableString("msg-ointocavel60", "Permaneça sem receber dano de predadores durante 60 segundos."));
	...
	stringstream intocavel_andamento;
	lb_intocavel_andamento = new TextNode("lb_intocavel_andamento");
	simdunas_cat.debug()<<"seconds untouched = "<<player_achiev.get_incont()<<endl;
	if (lvl_intocavel == 0) {
		intocavel_andamento<<player_achiev.get_incont();
		lb_intocavel_andamento->set_text(intocavel_andamento.str() + " / 60");
	}
	else if (lvl_intocavel == 1) {
		intocavel_andamento<<player_achiev.get_incont();
	...
	int lvl_guerreiro = player_achiev.get_geestrela();
	switch (lvl_guerreiro) {
		case 0:
			create_guerreiro_stars(white, white, white);
			hint_guerreiro_atual = new Hint(np_frame, lb_guerreiro, lb_guerreiro->get_np(), "hint_guerreiro", ConfigVariableString("msg-espante2", "Espante 2 lagartos machos (necessários 12 meses de vida)."));
	...
	lb_guerreiro_andamento = new TextNode("lb_guerreiro_andamento");
	if (lvl_guerreiro == 0) {
		guerreiro_andamento<<player_achiev.get_gecont();
		lb_guerreiro_andamento->set_text(guerreiro_andamento.str() + " / 2");
	}
	else if (lvl_guerreiro == 1) {
		guerreiro_andamento<<player_achiev.get_gecont();

...
NodePath test_model;
PT(MultiInstanceManager) manager;
...
void update_manager(const Event*, void*){
//	cout << "Updating Manager" << endl;
	manager->update();
}
...
//	test_model = window->load_model(render, "models/npcs/aranha_anim");
//
//	// Make the model 1unit wide
//	LPoint3 min, max;
//	test_model.calc_tight_bounds(min, max);
//	LVecBase3 _osize (max - min);
//	test_model.set_scale(1.0 / _osize[0]);
//	test_model.flatten_light();




	event_handler->add_hook(TimeControl::EV_pass_frame, update_manager, NULL);
	int count = 2048;
	manager = new MultiInstanceManager(count);
//	manager->register_model(test_model, count, true, true);
...
//	ModelRepository::get_instance()->get_model(name)->flatten_light();
...
class ModifiedPrey : public Prey {
public:
	ModifiedPrey(NodePath node) : Prey(node){}
	void change_size(const LVecBase3f &size) {
		_size = size;
	}
...
/*!
 * We need this hub because we do a neat trick for rendering prey.
 * We keep a static version of the model and a animated version.
 * Only one model actually exists. If the current prey, which will
 * be a InstanceHub is moving, we hide the static one.
 * This trick was design to improve performance.
 */
class InstanceHub : public Prey {
public:
//	InstanceHub(const string &name, const NodePath &prey) : PandaNode(name){
//		this->_prey = prey;
//	}

//	/* The animated and static versions of the model is under this node.
//	 * But the instance matrix can't be updated automatically, because we can
//	 * get even better performance, by not putting the InstanceNode's in render.
//	 */
//	virtual void transform_changed(){
//		int num_children = PandaNode::get_num_children();
//		for(int i = 0; i < num_children; i++){
//			PT(InstanceNode) instance = DCAST(InstanceNode, PandaNode::get_child(i));
//			if(instance) instance->_manager->update_matrix(instance->_index, PandaNode::get_transform()->get_mat());
//		}
//	}

	InstanceHub(NodePath node) : Prey(node){
		// Contruct it the normal way. Just a wapper now
	}
	...
	void update(/*const NodePath &np*/){
//		cout << "Updating InstanceHub" << endl;

//		for(int i = 0; i < instances.size(); i++){
//			cout << "Updating: " << i << endl;
////			cout << np.get_transform()->get_mat() << endl;
////			instances[i]->update();
////			 //instances[i]->_manager->update_matrix(instances[i]->_index, np.get_transform()->get_mat());
////			cout << "Capotou antes" << endl;
//		}


		for(int i = 0; i < get_num_children(); i++){
			get_child(i).set_transform(get_transform());
		}
	}

	//vector<PT(InstanceNode)> instances;
	//vector<NodePath> nps;
//	NodePath _prey;
};
...
//	NodePath tmp_model = *ModelRepository::get_instance()->get_animated_model(name);

	//NodePath model = NodePath("Prey Model for Instancing");

//	PT(AnimatedObjetoJogo) aoj = ModelRepository::get_instance()->get_model(name);
//	aoj->calc_size_from_bounds();
////	aoj->set_length(0.03, true);
//	aoj->loop_anim("character");
//	aoj->flatten_light();

	//NodePath model = tmp_model.copy_to(render);
	//DCAST(AnimatedObjetoJogo, model.node())->bind_anims(model.node());
	//model.flatten_light();

//	NodePath tmp_model = *ModelRepository::get_instance()->get_model(name);
//	NodePath model = *aoj;

	cout << "Quantidade: " << qtd << endl;

	// Configure Static Models
	ModelRepository::get_instance()->get_model(name)->calc_size_from_bounds();
	ModelRepository::get_instance()->get_model(name)->set_length(0.03, true);
	ModelRepository::get_instance()->get_model(name)->flatten_light();
	NodePath static_model = *ModelRepository::get_instance()->get_model(name);
	static_model.reparent_to(render);

	// Configure Dynamic Models
	ModelRepository::get_instance()->get_animated_model(name)->calc_size_from_bounds();
	ModelRepository::get_instance()->get_animated_model(name)->set_length(0.03, true);
	ModelRepository::get_instance()->get_animated_model(name)->flatten_light();
	ModelRepository::get_instance()->get_animated_model(name)->bind_anims(ModelRepository::get_instance()->get_animated_model(name)->node());
	ModelRepository::get_instance()->get_animated_model(name)->loop_anim("character");
	NodePath model = *ModelRepository::get_instance()->get_animated_model(name);
	model.reparent_to(render);

	// The model can't be obtained after
	LVecBase3f size = ModelRepository::get_instance()->get_animated_model(name)->get_size();


	// TODO: We need to better estimate the budget
	manager->register_model(static_model, /*qtd*/128, true, true);
	manager->register_model(model, /*qtd*/128, true, true);

	for(int i = 0; i < qtd; i++){
//		/* FUNCIONANDO! */
//		PT(InstanceNode) instance = manager->make_instance(model);
//		NodePath test = dummy.attach_new_node(instance);
//		test.set_scale(10);
//		test.set_pos(player->get_pos(render) + LPoint3f(rand()%10, rand()%10, 0));


		/* Cria nova instância de cada Presa. */
		PT(InstanceHub) hub = new InstanceHub(NodePath("Prey Hub Placeholder"));

		PT(InstanceNode) animated_instance = manager->make_instance(model);
		NodePath static_np = hub->attach_new_node(animated_instance);

		PT(InstanceNode) static_instance = manager->make_instance(static_model);
		NodePath animated_np = hub->attach_new_node(static_instance);
...
//
////				PT(Prey) c_npc = new ModifiedPrey(NodePath("Prey PlaceHolder"));
...
////				c_npc->attach_new_node(instanceNode);
////				c_npc->attach_new_node(instanceNodeStatic);
//				PT(Prey) c_npc = new ModifiedPrey(NodePath(instanceNode));
//				DCAST(ModifiedPrey, c_npc)->change_size(size);
//
...
//	cout << get_children() << endl;
//
//	// I'll assume the order won't change
//	cout << "Child 0" << get_child(0) << endl;
//	cout << "Child 1" << get_child(1) << endl;
